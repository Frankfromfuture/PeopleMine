...
const SPIRIT_ANIMAL_OPTIONS = [
  { value: 'TIGER',   emoji: '🐯', label: '老虎',   desc: '强势、果决、开拓型' },
  { value: 'PEACOCK', emoji: '🦚', label: '孔雀',   desc: '感染力强、善于表达' },
  { value: 'OWL',     emoji: '🦉', label: '猫头鹰', desc: '深度分析、冷静理性' },
  { value: 'KOALA',   emoji: '🐨', label: '考拉',   desc: '稳重可靠、温和包容' },
]
    ...
    <div className="space-y-6 max-w-2xl">
        ...
        <div className="p-6 grid grid-cols-2 gap-4">
          ...
          <label className="block col-span-2">
            ...
            <textarea
              ...
              rows={2}
              className="mt-1 w-full px-3 py-2 rounded-lg border border-gray-200 outline-none focus:ring-2 focus:ring-gray-100 focus:border-gray-400 text-sm resize-none"
            />
          </label>
        </div>
      </div>
        ...
        <div className="p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {selectedTags.filter(t => !TAG_SUGGESTIONS.includes(t)).map(tag => (
                <button
                  key={tag}
                  onClick={() => toggleTag(tag)}
                  className="px-3 py-1.5 text-sm rounded-full border bg-gray-600 text-white border-gray-600"
                >
                  {tag} ×
                </button>
              ))}
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 h-9 px-3 rounded-lg border border-gray-200 outline-none focus:ring-2 focus:ring-gray-100 focus:border-gray-400 text-sm"
            />
            <button
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="p-6">
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 rounded-lg border border-gray-200 outline-none focus:ring-2 focus:ring-gray-100 focus:border-gray-400 text-sm resize-none"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 pb-8">
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="px-6 py-2.5 rounded-lg bg-gray-600 text-white text-sm font-medium hover:bg-gray-700 disabled:opacity-50 transition"
        >
          {isSaving ? '保存中…' : '保存'}
        </button>
        {saved && (
          <span className="text-sm text-gray-600">✓ 已保存，航程分析将以此为起点</span>
        )}
      </div>
    </div>
  )
