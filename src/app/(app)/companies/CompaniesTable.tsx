...
const TEMP_COLOR: Record<Temperature, string> = {
  HOT: 'bg-gray-100 text-gray-700 border-gray-200',
  WARM: 'bg-gray-100 text-gray-700 border-gray-200',
...
export default function CompaniesTable({ companies }: { companies: Company[] }) {
  ...
  return (
    <div className="bg-white/90">
      <div className="border-b border-line-standard bg-slate-50/80 px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          <Link href="/companies/new" className="inline-flex items-center gap-1 rounded-md bg-gray-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-gray-500">+ 新建</Link>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索企业、行业、主营业务" className="h-8 w-56 rounded-md border border-slate-300 bg-white/90 px-2.5 text-xs outline-none focus:border-gray-400" />
          <select value={scaleFilter} onChange={(e) => setScaleFilter(e.target.value as 'ALL' | CompanyScale)} className="h-8 rounded-md border border-slate-300 bg-white/90 px-2 text-xs">
            <option value="ALL">全部规模</option>{Object.entries(COMPANY_SCALE_LABELS).map(([k, v]) => <option key={k} value={k}>{v.name}</option>)}
          </select>
          <select value={tempFilter} onChange={(e) => setTempFilter(e.target.value as 'ALL' | Temperature)} className="h-8 rounded-md border border-slate-300 bg-white/90 px-2 text-xs">
            <option value="ALL">全部温度</option><option value="HOT">热</option><option value="WARM">温</option><option value="COLD">冷</option>
          </select>
          <select value={groupBy} onChange={(e) => setGroupBy(e.target.value as GroupBy)} className="h-8 rounded-md border border-slate-300 bg-white/90 px-2 text-xs">
          ...
          <details className="relative">
            <summary className="flex h-8 cursor-pointer list-none items-center rounded-md border border-slate-300 bg-white/90 px-2.5 text-xs text-slate-700 hover:bg-slate-50">自定义列</summary>
            <div className="absolute left-0 top-9 z-30 w-44 rounded-md border border-line-standard bg-white/90 p-2 shadow-lg">
          ...
          <span className="ml-auto text-xs text-text-secondary">共 {filtered.length} 条 · 可拖拽列头排序/拉伸宽度</span>
        </div>

        <div className="mt-2 flex flex-wrap items-center gap-2">
          <button onClick={() => setActiveView('all')} className={`px-2.5 py-1 text-xs rounded-md border ${activeView === 'all' ? 'bg-gray-600 text-white border-gray-600' : 'bg-white/90 text-slate-600 border-slate-300'}`}>全部</button>
          <button onClick={() => setActiveView('core')} className={`px-2.5 py-1 text-xs rounded-md border ${activeView === 'core' ? 'bg-gray-600 text-white border-gray-600' : 'bg-white/90 text-slate-600 border-slate-300'}`}>核心企业</button>
          <button onClick={() => setActiveView('hot')} className={`px-2.5 py-1 text-xs rounded-md border ${activeView === 'hot' ? 'bg-gray-600 text-white border-gray-600' : 'bg-white/90 text-slate-600 border-slate-300'}`}>高温企业</button>
          ...
          <button onClick={deleteAll} disabled={isDeletingAll} className="px-2.5 py-1 text-xs rounded-md border border-gray-300 text-gray-700 disabled:opacity-50">
            全部删除
          </button>

          <button onClick={saveCurrentView} className="px-2.5 py-1 text-xs rounded-md border border-gray-300 text-gray-700">
            保存当前视图
          </button>
          ...
          <details className="relative">
            ...
            <div className="absolute left-0 top-8 z-30 w-64 rounded-md border border-line-standard bg-white/90 p-2 shadow-xl space-y-1">
              ...
              {savedViews.map((view) => (
                <div key={view.id} className="flex items-center gap-1">
                  ...
                  <button onClick={() => renameSavedView(view.id, view.name)} className="px-2 py-1 text-xs text-gray-600">改</button>
                  <button onClick={() => removeSavedView(view.id)} className="px-2 py-1 text-xs text-gray-600">删</button>
                </div>
              ))}
            </div>
          </details>
        ...
        {batchPanelOpen && (
          <div className="mt-2 p-3 rounded-lg border border-gray-200 bg-gray-50/50 flex flex-wrap items-center gap-2">
            ...
            }} className="h-8 rounded-md border border-gray-200 bg-white/90 px-2 text-xs">
            ...
            {batchField === 'temperature' && (
              <select value={batchValue} onChange={(e) => setBatchValue(e.target.value)} className="h-8 rounded-md border border-gray-200 bg-white/90 px-2 text-xs">
            ...
            <button disabled={isBatching || selectedIds.length === 0} onClick={applyBatch} className="px-3 py-1.5 text-xs rounded-md bg-gray-600 text-white disabled:opacity-40">
        ...
        {undoStack.length > 0 && (
          <div className="mt-2 p-2 rounded-md border border-gray-300 bg-gray-50 space-y-1">
            <p className="text-xs text-gray-800">可撤销最近批量编辑（10 秒内，最多 3 条）</p>
            {undoStack.map((entry, idx) => (
              <div key={entry.id} className="flex items-center justify-between">
                <span className="text-xs text-gray-700">操作 #{undoStack.length - idx}</span>
                <button onClick={() => undoBatch(entry.id)} className="px-2.5 py-1 text-xs rounded border border-gray-300 text-gray-800">
                  撤销
                </button>
              </div>
            ))}
        ...
        {deleteUndoStack.length > 0 && (
          <div className="mt-2 p-2 rounded-md border border-gray-300 bg-gray-50 space-y-1">
            <p className="text-xs text-gray-800">可撤销删除（最多保留 5 次）</p>
            {deleteUndoStack.map((entry, idx) => (
              <div key={entry.id} className="flex items-center justify-between gap-2">
                ...
                <button onClick={() => undoDelete(entry.id)} className="px-2.5 py-1 text-xs rounded border border-gray-300 text-gray-800">
                  撤销删除
                </button>
              </div>
            ))}
        ...
        {(undoSync.running || undoSync.total > 0) && (
          <div className="mt-2 p-2 rounded-md border border-gray-300 bg-gray-50 flex items-center justify-between gap-2">
            ...
            {undoSync.failedRows.length > 0 && !undoSync.running && (
              <button onClick={retryUndoSync} className="px-2.5 py-1 text-xs rounded border border-gray-300 text-gray-800">
                重试失败项
              </button>
            )}
          </div>
      ...
      {filtered.length === 0 ? <div className="px-6 py-14 text-center text-sm text-text-secondary">暂无匹配企业</div> : (
        <div className="overflow-x-auto">
          {grouped.map((group) => (
            <div key={group.key} className="border-b border-line-standard">
              <div className="sticky left-0 z-10 border-b border-line-standard bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-700">{group.title}</div>
              <table className="w-full text-sm" style={{ minWidth: shownColumns.reduce((n, c) => n + columnWidths[c.key], 0) }}>
                <thead><tr className="bg-white/90">
                  <th className="border-b border-r border-line-standard px-2 py-2 text-left text-xs font-semibold text-text-secondary" style={{ width: 110, minWidth: 110 }}>
                    操作
                  </th>
                  <th className="border-b border-r border-line-standard px-2 py-2 text-left text-xs font-semibold text-text-secondary" style={{ width: 44, minWidth: 44 }}>
                    选
                  </th>
                  {shownColumns.map((col) => (
                    <th key={col.key} draggable onDragStart={(e) => e.dataTransfer.setData('text/col', col.key)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDropColumn(e.dataTransfer.getData('text/col') as ColumnKey, col.key)} className="relative border-b border-r border-line-standard px-3 py-2 text-left text-xs font-semibold text-text-secondary" style={{ width: columnWidths[col.key], minWidth: columnWidths[col.key] }}>
                    ...
                    <tr key={row.id} className="hover:bg-gray-50/40">
                      <td className="border-b border-r border-slate-100 px-2 py-2">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => deleteOne(row)} className="rounded border border-gray-300 px-1.5 py-0.5 text-[11px] text-gray-700 hover:bg-gray-50">删除</button>
                        </div>
                      </td>
                      <td className="border-b border-r border-slate-100 px-2 py-2">
                        <input type="checkbox" checked={selectedIds.includes(row.id)} onChange={() => toggleSelect(row.id)} />
                      </td>
                      {visibleColumns.name && <td className="border-b border-r border-slate-100 px-3 py-2 font-medium text-slate-800"><Link href={`/companies/${row.id}`} className="hover:text-gray-700">{row.name}{isAiGeneratedCompany(row) ? '*' : ''}</Link></td>}
                      ...
                      {visibleColumns.familiarityLevel && <td className="border-b border-r border-slate-100 px-3 py-2 text-gray-500">{row.familiarityLevel ? '★'.repeat(row.familiarityLevel) : <span className="text-slate-400">—</span>}</td>}
